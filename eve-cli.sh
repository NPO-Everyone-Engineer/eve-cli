#!/bin/bash
# eve-cli.sh
# ローカルLLM (Ollama) で eve-coder を起動するスクリプト
...
#   eve-cli                    # インタラクティブモード
#   eve-cli -p "質問"          # ワンショット
#   eve-cli --auto             # ネットワーク状況で自動判定
#   eve-cli --model qwen3:8b   # モデル手動指定
#   eve-cli -y                 # パーミッション確認スキップ (自己責任)
#   eve-cli --debug            # デバッグモード

# NOTE: set -e を使わない (途中停止を防ぐ)
set -uo pipefail

# --- ディレクトリ初期化 ---
STATE_DIR="${HOME}/.local/state/eve-cli"
...
if [ ! -f "$EVE_CODER_SCRIPT" ]; then
    ...
    if [ -n "$SCRIPT_DIR" ] && [ -f "${SCRIPT_DIR}/eve-coder.py" ]; then
        ...
        echo "エラー: eve-coder.py が見つかりません"
        echo "  install.sh を実行するか、eve-coder.py を同じディレクトリに置いてください"
        exit 1
    fi
fi
...
if [ "$YES_FLAG" -eq 1 ]; then
    ...
    echo "============================================"
    echo ""
    echo " eve-cli はツール自動許可モード (-y) で起動できます。"
    echo ""
...
DEBUG_ARGS=()
if [ "$EVE_CLI_DEBUG" = "1" ] || [ "$EVE_CLI_DEBUG" = "true" ]; then
    DEBUG_ARGS+=(--debug)
fi
...
echo " Engine: eve-coder.py (direct, no proxy)"
echo "============================================"
echo ""
