...
MSG_ja_reopen="新しいターミナルを開いてから eve-cli を実行"
...
MSG_en_reopen="Open a new terminal, then run eve-cli"
...
MSG_zh_reopen="打开新终端后运行 eve-cli"
...
echo -e "${NC}${CYAN}${BOLD}"
cat << 'LOGO2'
        Everyone.Engineer Coding Agent
LOGO2
echo -e "${NC}"
echo -e "  ${PINK}💜${MAGENTA}💜${PURPLE}💜${CYAN}💜${AQUA}💜${MINT}💜${NEON_GREEN}💜${YELLOW}💜${ORANGE}💜${CORAL}💜${HOT_PINK}💜${PINK}💜${MAGENTA}💜${PURPLE}💜${CYAN}💜${AQUA}💜${NC}"
echo ""
...
if command -v claude &>/dev/null; then
    ...
    vapor_info "Claude Code CLI 🤖 not installed (optional — eve-coder replaces it)"
fi

# --- Python3 (REQUIRED for eve-coder) ---
...
vapor_success "eve-coder.py → $LIB_DIR/"
vapor_success "Command → $BIN_DIR/eve-cli"
...
CONFIG_DIR="${HOME}/.config/eve-cli"
...
# eve-coder.py syntax check
if python3 -c "import ast, sys; ast.parse(open(sys.argv[1]).read())" "$LIB_DIR/eve-coder.py" 2>/dev/null; then
    vapor_success "eve-coder.py       → 🟢 $(msg ready)"
else
    vapor_warn "eve-coder.py       → 🟡 $(msg warning) (syntax error)"
fi
...
echo ""
echo -e "    ${PINK}❯${NC} ${BOLD}${CYAN}eve-cli${NC}                     ${DIM}$(msg mode_interactive)${NC}"
echo -e "    ${PINK}❯${NC} ${BOLD}${CYAN}eve-cli -p \"...\"${NC}            ${DIM}$(msg mode_oneshot)${NC}"
echo -e "    ${PINK}❯${NC} ${BOLD}${CYAN}eve-cli --auto${NC}              ${DIM}$(msg mode_auto)${NC}"
echo ""
rainbow_text "    ═══════════════════════════════════════════════════════"
echo ""
...
echo -e "    ${PURPLE}┃${NC} $(msg label_command):   ${AQUA}$BIN_DIR/eve-cli${NC}"
echo ""
rainbow_text "    ═══════════════════════════════════════════════════════"
echo ""
...
if [ -n "${SHELL_RC:-}" ]; then
    echo -e "    ${BOLD}source ${SHELL_RC} && eve-cli${NC}"
else
    echo -e "    ${BOLD}export PATH=\"\${HOME}/.local/bin:\${PATH}\" && eve-cli${NC}"
fi
echo ""
echo ""
