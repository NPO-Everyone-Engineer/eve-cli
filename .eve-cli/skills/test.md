...
- **eve-coder.py のテストは単体インポートが困難**: 約8500行の単一ファイルで、クラスやグローバル変数の初期化時に副作用（readline 初期化、ターミナル制御等）がある。テストでは `unittest.mock.patch` や `importlib` を使った部分的なインポートが必要
- **標準ライブラリのみでテスト**: pytest は開発用に許容されているが、テスト対象コード自体は外部ライブラリに依存してはならない。テストコードで外部ライブラリを使うのは OK
- **既存テストとの整合性**: `tests/` ディレクトリに既存のテストファイルがある場合、そのスタイル（クラスベース vs 関数ベース、フィクスチャの使い方等）に合わせること
- **macOS 環境固有のテスト失敗**: readline（libedit）関連、パスの日本語文字、プロセス制御（signal 等）のテストは OS によって挙動が異なる。CI と開発環境の差異に注意
