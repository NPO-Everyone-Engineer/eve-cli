...
EvE CLI の日本語 UX が大幅に強化されました。エラーメッセージ、ヘルプ、スラッシュコマンドのすべてが日本語で表示されます。

### 実装内容

| カテゴリ | 件数 | 内容 |
|---------|------|------|
| **エラーメッセージ** | 80 | 引数検証、Ollama 通信、MCP、サイドカー、セッション管理など |
| **ヘルプメッセージ** | 32 | `--help` 出力（基本オプション、ループ、RAG、学習モードなど） |
| **スラッシュコマンド** | 36 | Usage 14 件 + 出力 22 件（/save, /fork, /compact, /commit など） |
| **ドキュメント** | 5 | すべて事前日本語化済み（getting-started, usage, commands など） |
| **合計** | **153** | ✅ 完了 |
...
```bash
# ヘルプが日本語で表示
$ LANG=ja_JP.UTF-8 python3 eve-coder.py --help

EvE CLI — Everyone.Engineer コーディングエージェント（Ollama ローカル + クラウド）

options:
  -p, --prompt          ワンショットプロンプト（非インタラクティブ）
  -m, --model           Ollama モデル名
  --max-agent-steps     リクエストあたりの最大 AI 内部ステップ数
                        （デフォルト：100, 最大：200）

# スラッシュコマンドも日本語
> /git
使用例：/git <コマンド>（例：/git log --oneline -10）

> /compact
圧縮しました：1000 → 500 tokens
```

### 技術詳細

- **翻訳システム**: `locales/ja.json` + `t()` 関数
- **自動言語検出**: `LANG`, `LC_ALL`, `LC_MESSAGES`, `EVE_CLI_LANG`
- **テスト**: 53 件すべて合格
...
## README では省略しているけれど、実装されている主な機能

README を読みやすくするため細部は省いていますが、実装上は次のような機能もあります。

- セッション一覧表示（`--list-sessions`）
- 特定セッションの再開（`--session-id`）
- `stream-json` 出力
- RAG 関連オプション（`--rag`, `--rag-index` など）
- `/watch`, `/autotest`, `/save`, `/fork`
- `/theme`, `/hooks`, `/skills`, `/browser status`
- `/index def`, `/index refs`, `/index status`
- カスタムコマンド（`/custom`, `/cmd`）

必要になったタイミングで [コマンドリファレンス](docs/commands.md) や [高度な機能](docs/advanced.md) を開くのがおすすめです。
