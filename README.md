...
- **ステップ上限の設定**: 長いタスク向けに、1リクエストあたりのAI内部ステップ上限を `--max-agent-steps` で調整可能
...
CLI の改善計画とキャッチアップ方針は [00_Docs/20260308_cli_catchup_instruction.md](00_Docs/20260308_cli_catchup_instruction.md) にまとめています。
...
# 長めのタスク向けに内部ステップ上限を増やす（既定: 50 / 上限: 200）
eve-cli --max-agent-steps 80
...
設定ファイル（`~/.config/eve-cli/config`）や環境変数でも指定できます。
...
# 環境変数
EVE_CLI_MAX_AGENT_STEPS=80 eve-cli
```
