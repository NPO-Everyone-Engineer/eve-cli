...
- [特徴](#特徴)
- [CLI キャッチアップ計画（Codex / Claude / Gemini）](#cli-キャッチアップ計画 codex--claude--gemini)
- [クイックスタート](#クイックスタート)
- [インストール](#インストール)
- [基本的な使い方](#基本的な使い方)
- [入力方法](#入力方法)
...
詳細な指示書：[`00_Docs/20260308_cli_catchup_instruction.md`](00_Docs/20260308_cli_catchup_instruction.md)

### P0（最優先：必須機能）

| 機能 | 概要 | ステータス |
|------|------|-----------|
| **マルチプロバイダ標準対応** | Ollama + OpenAI/Anthropic/Google 系 API を切替可能に | 設計済（実装待機） |
| **通知の実運用強化** | セッション完了・エラー時の通知を cmux/tmux/OSC で標準化 | ✅ 実装済（v2.2.0） |
| **非対話モード強化** | CI/CD 向けの安定した JSON 出力と終了コード標準化 | ✅ 実装済（v2.2.1） |
| **失敗時リカバリ（Resume/Replay）** | 実行履歴・checkpoint からの再開 | ✅ 実装済（`--resume` / `/checkpoint` / `/rollback`） |
| **ループモード時間制限** | 長時間自律実行のための時間制限機能 | ✅ 実装済（v2.2.0 `--max-loop-hours`） |

### P1（競争力を高める重要項目）

- コンテキスト理解の深掘り（依存グラフ・影響範囲分析）
- レビュー/修正ループの品質評価
- PR 中心ワークフローの統合
...
> 本計画は段階的に実装を進めるロードマップです。最新の進捗は上記指示書を参照してください。
